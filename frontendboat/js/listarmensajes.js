...
function listarRespuesta(items) {
    ...
    for (var i=0; i < items.length; i++) {
        var texto = `Message: ${items[i].messageText}</br>Client: ${items[i].client.name}</br>Boat: ${items[i].boat.name}`;
        tabla +=`<tr>
                   <td>${items[i].messageText}</td>
                   <td>${items[i].client.name}</td>
                   <td>${items[i].boat.name}</td>
                   <td><button onclick="mostrarmensaje()">Edit</button></td>
                   <td><button onclick="mostrarEliminar(${items[i].idMessage},'${texto}')">Delete</button></td>
                   </tr>`;
        texto="";
    }
